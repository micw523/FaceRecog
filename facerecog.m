...
% 120 dictionary elements, 50 iterations, 30 max. nonzero elements
% per image maximum
...
train.X = sparse_X; train.y = im_label_train;
...
n = size(dic_mtx,2)+1;
theta = randn(n,num_classes-1)*0.05;
...
test.X = sp_X_test; test.y = im_label_test;
test.X = [ones(1,size(test.X,2)); test.X];
accuracy = multi_classifier_accuracy(theta,test.X,test.y);
fprintf('Test accuracy: %2.1f%%\n', 100*accuracy);

%% SVM Classifier
train.X = full(train.X);
test.X = full(test.X);
[test_acc, train_acc, ~] = svm_classifier_simple(train.X(2:end,:)',train.y',...
    test.X(2:end,:)',test.y');
fprintf('Training accuracy: %2.1f%%\n', 100*train_acc);
...
for i = 1:num_test_case
    ...
    SCI(i) = ((kk*max(abs(sp_indices))/norm(sp_indices,1)-1)/(kk-1))>tau;
end
acc_rate = nnz(SCI)/num_test_case;
fprintf('At tau = %0.2f, %f%% was accepted as valid\n', tau, acc_rate*100);
sp_X_test_sci = sp_X_test(:,SCI);
im_label_test_sci = im_label_test(SCI);

%% Process test set selected by SCI
test.X = full(sp_X_test_sci); test.y = im_label_test_sci;
test.X = [ones(1,size(test.X,2)); test.X];
accuracy = multi_classifier_accuracy(theta,test.X,test.y);
fprintf('Test accuracy: %2.1f%%\n', 100*accuracy);
